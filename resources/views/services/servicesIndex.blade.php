    ...
    @section('content')
    <div >
        <a href="/admin/serviceapps" class="btn btn-success" style="margin-right: 1rem">
            সকল
        </a>
        @foreach (\Helper::applicationStatus() as $key =>$value)
            <a href="/admin/serviceapps/details-status/{{$value}}" style="margin-right: 1rem" class="btn
            @if($value == "পেন্ডিং")
            btn-info
      ...
      <a href="/admin/addservice/upload" class="btn btn-success" style="margin-top: 1rem">
        Upload Union Photo
    </a>
...
<th>Action </th>
    ...
    <tbody>
        ...
        <tr id="el{{$service->id}}">
            ...
            <td>
                <a class="btn btn-xs btn-primary btn-detail" title="Detail Data" onClick="printFunc(`/admin/serviceapps/detail/{{$service->id}}`)">download</a>
